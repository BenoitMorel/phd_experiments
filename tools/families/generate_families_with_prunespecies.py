...
def extract_family(family, inputdir, outputdir, species_to_keep):
  ...
  input_gene_tree = fam.get_true_tree(inputdir, family)
  if (os.path.isfile(input_gene_tree)):
    tree = Tree(input_gene_tree, format = 1)
    tree.prune(genes_to_keep)
    tree.write(outfile = fam.get_true_tree(outputdir, family))
...
def generate(inputdir, outputdir, species_to_prune):
  ...
  for family in families:
    extract_family(family, inputdir, outputdir, species_to_keep)
...
if (__name__ == "__main__"): 
  if (len(sys.argv) < 3): 
    print("Syntax: python " + os.path.basename(__file__) + " input output species1 [species2 species3 ...]")
    exit(1)
  inputdir = sys.argv[1]
  outputdir = sys.argv[2]
  species_to_prune = sys.argv[3:]
  generate(inputdir, outputdir, species_to_prune)
