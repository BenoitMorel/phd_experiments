...
def init_gene_trees_file(datadir, subst_model, output_dir):
  ...
  with open(filepath, "w") as writer:
    for family in fam.get_families_list(datadir):
      ...
      print("TODO: remove scientific notation")
      m = {}
...
def exec_duptree(gene_trees_file):
  command = []
  ...
  res = subprocess.check_output(command, stderr=sys.stdout.buffer)
  return res.split("\n")[-3]
...
def run_duptree(datadir, subst_model):
  ...
  start = time.time()
  species_tree_str = exec_duptree(gene_trees_file)
